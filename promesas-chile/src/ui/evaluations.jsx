...
function EvalRecorder({ config, onSave, onCancel }) {
  ...
  const lap = () => {
    const prev = data[active].laps.reduce((a, b) => a + b, 0);
  ...
  const hasData = Object.values(data).some(d => (d.laps || d.tramos).length > 0);

  const save = () => {
    const clean = {};
    config.swimmerIds.forEach(id => {
      if (config.metrica === 'cronometro') {
        const laps = data[id].laps;
        clean[id] = { laps, total: +laps.reduce((a, b) => a + b, 0).toFixed(2) };
      } else {
        clean[id] = { tramos: data[id].tramos };
      }
    });
    DB.addEvaluation({ fecha: todayISO(), protocolo: config.protocolo, metrica: config.metrica, swimmerIds: config.swimmerIds, tecnico: st.coach.nombre, notas, data: clean });
    ...
    <div>
      ...
      <div className="rec-swimmers">
        {swimmers.map(s => {
          ...
          return (
            <button key={s.id} className="rec-swim" data-on={active === s.id} onClick={() => { setActive(s.id); setCounter(0); }}>
        ...
        {config.metrica === 'cronometro' ? (
          <>
            ...
            <div className="card">
              ...
              {(!activeData.laps || activeData.laps.length === 0) ? <Empty icon="flag" title="Sin vueltas" >Pulsa “Vuelta” para registrar cada tramo.</Empty> : (
                <table className="tbl">
                  <thead><tr><th>#</th><th>Parcial</th><th>Acumulado</th><th>vs objetivo</th></tr></thead>
                  <tbody>
                    {activeData.laps.map((l, i) => {
                      const acc = activeData.laps.slice(0, i + 1).reduce((a, b) => a + b, 0);
                      const diff = target ? l - target : null;
                      return (
                        <tr key={i}>
                          <td style={{ fontWeight: 700, color: 'var(--ink)' }}>{i + 1}</td>
                          <td style={{ fontVariantNumeric: 'tabular-nums', fontWeight: 700, fontSize: 15 }}>{fmtTime(l)}</td>
              ...
              {proto.niveles && activeData.laps && activeData.laps.length > 0 && (
                <div className="card-pad" style={{ borderTop: '1px solid var(--line-soft)' }}>
                  <NivelBadge proto={proto} avgTime={activeData.laps.reduce((a, b) => a + b, 0) / activeData.laps.length} />
                </div>
              )}
            </div>
          </>
          ...
          <>
            <div className="card card-pad rec-stage">
              ...
              <Button variant="dark" icon="check" style={{ marginTop: 18 }} disabled={counter === 0} onClick={registerTramo}>Registrar tramo {activeData.tramos.length + 1}</Button>
            </div>
            <div className="card">
                ...
                <div className="card-pad">
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(64px,1fr))', gap: 10 }}>
                    {activeData.tramos.map((t, i) => (
                      <div key={i} style={{ textAlign: 'center', padding: '12px 8px', background: 'var(--surface-2)', border: '1px solid var(--line-soft)', borderRadius: 'var(--r-md)' }}>
                  ...
                  <div style={{ marginTop: 14, fontSize: 13, color: 'var(--slate-500)', fontWeight: 600 }}>
                    Promedio: <strong style={{ color: 'var(--ink)' }}>{(activeData.tramos.reduce((a, b) => a + b, 0) / activeData.tramos.length).toFixed(1)}</strong> · Total tramos: {activeData.tramos.length}
                  </div>
                </div>
              )}
            </div>
...
function CompoundRecorder({ config, onSave, onCancel }) {
  ...
  const hasData = Object.values(data).some(d => d.intentos.length > 0);
  const canRegister = sw.elapsed > 0 && !sw.running;

  const save = () => {
    DB.addEvaluation({ fecha: todayISO(), protocolo: config.protocolo, metrica: 'compuesta', swimmerIds: config.swimmerIds, tecnico: st.coach.nombre, notas, data });
  ...
  return (
    <div>
      ...
      <div className="rec-swimmers">
        {swimmers.map(s => (
          <button key={s.id} className="rec-swim" data-on={active === s.id} onClick={() => { setActive(s.id); resetEntry(); }}>
            <Avatar name={s.nombre} size={30} />
            <span className="rs-name">{s.nombre.split(' ').slice(0, 2).join(' ')}</span>
      ...
      <div className="rec-body">
        <div className="card card-pad rec-stage">
          ...
          <Button variant="dark" icon="check" style={{ marginTop: 20 }} disabled={!canRegister} onClick={registrar}>
        ...
        <div className="card">
          ...
          {activeData.intentos.length === 0 ? <Empty icon="timer" title="Sin intentos">Cronometra el tiempo y cuenta, luego registra el intento.</Empty> : (
            <table className="tbl">
              <thead><tr><th>#</th><th>Tiempo</th>{proto.metricas.map(m => <th key={m}>{DB.METRIC_TYPES[m].label}</th>)}</tr></thead>
              <tbody>
                {activeData.intentos.map((it, i) => (
                  <tr key={i}>
                    <td style={{ fontWeight: 700, color: 'var(--ink)' }}>{i + 1}</td>
                    <td style={{ fontVariantNumeric: 'tabular-nums', fontWeight: 700, fontSize: 15 }}>{fmtTime(it.tiempo)}</td>
                    {proto.metricas.map(m => <td key={m} style={{ fontWeight: 700, color: DB.METRIC_TYPES[m].color }}>{it[m]}</td>)}
                  </tr>
                ))}
              </tbody>
            </table>
...
function MultiRecorder({ config, onSave, onCancel }) {
  ...
  const corr = (raw, id) => Math.max(0, +(raw - (offsets[id] || 0)).toFixed(2)); // corrected for stagger
  ...
  const totalLaps = Object.values(data).reduce((n, d) => n + d.laps.length, 0);
  const allDone = swimmers.every(s => data[s.id].laps.length >= reps);
  const hasData = totalLaps > 0;
  const allTapped = swimmers.every(s => tapped[s.id] || data[s.id].laps.length >= reps);

  const save = () => {
    const clean = {};
    config.swimmerIds.forEach(id => { const laps = data[id].laps.map(r => corr(r, id)); clean[id] = { laps, total: +laps.reduce((a, b) => a + b, 0).toFixed(2) }; });
    DB.addEvaluation({ fecha: todayISO(), protocolo: config.protocolo, metrica: 'cronometro', swimmerIds: config.swimmerIds, tecnico: st.coach.nombre, notas, data: clean, modo: 'simultaneo', descansos: rests, desfases: offsets });
    toast('Evaluación guardada'); onSave();
  };
    ...
    <div>
      <div className="rec-head card card-pad">
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          ...
          <div>
            <div style={{ fontSize: 12.5, color: 'var(--slate-500)', fontWeight: 600 }}>Cronómetro simultáneo · {swimmers.length} nadadores</div>
            <h2 style={{ fontSize: 19 }}>{proto.nombre}</h2>
          </div>
          <span className="badge badge-navy" style={{ marginLeft: 6 }}>{proto.set}</span>
          <div style={{ flex: 1 }} />
      ...
      <div className="multi-grid">
        {swimmers.map(s => {
          ...
          const total = lapsC.reduce((a, b) => a + b, 0);
          const last = lapsC[lapsC.length - 1];
          ...
          return (
            <div key={s.id} className="multi-card" data-done={done} data-armed={armed} data-tapped={isTapped}>
              ...
              <div className="mc-time" style={{ color: isTapped ? 'var(--green)' : undefined }}>{last != null ? fmtTime(last) : '0:00.00'}</div>
              ...
              {reps > 1 && laps.length > 0 && (
                <div className="mc-laps">
                  {lapsC.map((l, i) => <span key={i} className="mc-lap">{fmtTime(l)}</span>)}
                </div>
              )}
            </div>
          );
...
function IntervalRecorder({ config, onSave, onCancel }) {
  ...
  const hasData = Object.values(data).some(d => d.reps.length > 0);
  const save = () => {
    const clean = {};
    config.swimmerIds.forEach(id => {
      const r = data[id].reps;
      clean[id] = { reps: r, totalNado: +r.reduce((a, b) => a + (b.nado || 0), 0).toFixed(2), totalCiclo: +r.reduce((a, b) => a + (b.nado || 0) + (b.descanso || 0), 0).toFixed(2) };
    });
    DB.addEvaluation({ fecha: todayISO(), protocolo: config.protocolo, metrica: 'cronometro', swimmerIds: config.swimmerIds, tecnico: st.coach.nombre, notas, data: clean });
    toast('Evaluación guardada'); onSave();
  };
  ...
  const totalNado = reps.reduce((a, b) => a + (b.nado || 0), 0);
  ...
  return (
    <div>
      ...
      <div className="rec-swimmers">
        {swimmers.map(s => (
          <button key={s.id} className="rec-swim" data-on={active === s.id} onClick={() => switchTo(s.id)}>
            <Avatar name={s.nombre} size={30} />
            <span className="rs-name">{s.nombre.split(' ').slice(0, 2).join(' ')}</span>
      ...
      <div className="rec-body">
        <div className="card card-pad rec-stage">
          ...
          <div className="rec-controls">
            ...
            {phase === 'nado' && <Button onClick={llegada} variant="dark" icon="flag" className="big-btn">Llegada (fin nado)</Button>}
            {phase === 'descanso' && <>
          ...
          {reps.length > 0 && (
            <div style={{ marginTop: 18, display: 'flex', gap: 14, fontSize: 13, color: 'var(--slate-500)', fontWeight: 600, alignItems: 'center', flexWrap: 'wrap', justifyContent: 'center' }}>
              ...
              {proto.niveles && <NivelBadge proto={proto} avgTime={totalNado / reps.length} size="sm" />}
            </div>
          )}
        </div>
        ...
        <div className="card">
          ...
          {reps.length === 0 ? <Empty icon="timer" title="Sin repeticiones">Inicia la serie: cronometra el nado, marca la llegada y la salida.</Empty> : (
            <table className="tbl">
              <thead><tr><th>#</th><th>Nado</th><th>Descanso</th><th>Ciclo</th>{ciclo && <th>vs salida</th>}</tr></thead>
              <tbody>
                {reps.map((r, i) => {
                  const cic = (r.nado || 0) + (r.descanso || 0);
                  const diff = ciclo && r.descanso != null ? cic - ciclo : null;
                  return (
                    <tr key={i}>
                      <td style={{ fontWeight: 700, color: 'var(--ink)' }}>{i + 1}</td>
                      <td style={{ fontVariantNumeric: 'tabular-nums', fontWeight: 700, fontSize: 15, color: 'var(--cyan-700)' }}>{fmtTime(r.nado)}</td>
                      ...
                      <td style={{ fontVariantNumeric: 'tabular-nums', color: 'var(--slate-500)' }}>{r.descanso != null ? fmtTime(cic) : '—'}</td>
            ...
            {ev.metrica === 'cronometro' ? (
              d.reps ? (
                ...
                <table className="tbl" style={{ border: '1px solid var(--line-soft)', borderRadius: 'var(--r-md)' }}>
                  <thead><tr><th>#</th><th>Nado</th><th>Descanso</th><th>Ciclo</th></tr></thead>
                  <tbody>
                    {d.reps.map((r, i) => (
                      <tr key={i}>
                        <td style={{ fontWeight: 700, color: 'var(--ink)' }}>{i + 1}</td>
                        <td style={{ fontVariantNumeric: 'tabular-nums', fontWeight: 700, color: 'var(--cyan-700)' }}>{fmtTime(r.nado)}</td>
                        ...
                        <td style={{ fontVariantNumeric: 'tabular-nums', color: 'var(--slate-500)' }}>{r.descanso != null ? fmtTime((r.nado || 0) + r.descanso) : '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {(d.laps || []).map((l, i) => <span key={i} className="lap-chip">{i + 1}<b>{fmtTime(l)}</b></span>)}
                  </div>
                  {proto.niveles && d.laps && d.laps.length > 0 && (
                    <div style={{ marginTop: 10 }}><NivelBadge proto={proto} avgTime={d.laps.reduce((a, b) => a + b, 0) / d.laps.length} size="sm" /></div>
                  )}
                </>
              )
                ...
                <table className="tbl" style={{ border: '1px solid var(--line-soft)', borderRadius: 'var(--r-md)' }}>
                  ...
                  <tbody>
                    {d.intentos.map((it, i) => (
                      <tr key={i}>
                        <td style={{ fontWeight: 700, color: 'var(--ink)' }}>{i + 1}</td>
                        <td style={{ fontVariantNumeric: 'tabular-nums', fontWeight: 700 }}>{fmtTime(it.tiempo)}</td>
                        {proto.metricas.map(m => <td key={m} style={{ fontWeight: 700, color: DB.METRIC_TYPES[m].color }}>{it[m]}</td>)}
                      </tr>
                    ))}
                  </tbody>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {(d.tramos || []).map((t, i) => <span key={i} className="lap-chip">T{i + 1}<b>{t}</b></span>)}
              </div>
            )}
          </div>
        );
      })}
      {ev.notas && <div style={{ marginTop: 8, padding: 14, background: 'var(--surface-2)', borderRadius: 'var(--r-md)', fontSize: 13.5, color: 'var(--slate-700)', lineHeight: 1.55 }}><strong style={{ color: 'var(--ink)' }}>Notas: </strong>{ev.notas}</div>}
    </Modal>
  );
}
...
function ManualEvalForm({ onSave, onCancel }) {
  ...
  const hasData = sel.length > 0 && sel.some(id => {
    const e = entryOf(id);
    if (plan.kind === 'reps') return e.reps.some(r => parseTime(r.nado) != null);
    if (plan.kind === 'laps') return e.laps.some(l => parseTime(l) != null);
    if (plan.kind === 'tramos') return e.tramos.some(t => String(t).trim() !== '');
    return e.intentos.some(it => parseTime(it.tiempo) != null);
  });
  ...
  const save = () => {
    ...
    sel.forEach(id => {
      const e = entryOf(id);
      if (plan.kind === 'reps') {
        const reps = e.reps.map(r => ({ nado: parseTime(r.nado), descanso: parseTime(r.descanso) })).filter(r => r.nado != null);
        finalData[id] = { reps, totalNado: reps.reduce((a, b) => a + (b.nado || 0), 0) };
      } else if (plan.kind === 'laps') {
        const laps = e.laps.map(parseTime).filter(v => v != null);
        finalData[id] = { laps, total: laps.reduce((a, b) => a + b, 0) };
      } else if (plan.kind === 'tramos') {
        finalData[id] = { tramos: e.tramos.map(v => parseInt(v, 10)).filter(v => !isNaN(v)) };
      } else {
        const intentos = e.intentos.map(it => {
          const o = { tiempo: parseTime(it.tiempo) };
          plan.metricas.forEach(m => { o[m] = parseInt(it[m], 10) || 0; });
          return o;
        }).filter(it => it.tiempo != null);
  ...
  const tHint = 'Formato m:ss.cc — ej. 1:28.40 ó 88.4';
                ...
                {plan.kind === 'tramos' && (
                  <>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                      <div style={{ fontSize: 11.5, color: 'var(--slate-500)' }}>Conteo de {metrica === 'brazadas' ? 'brazadas' : 'movimientos subacuáticos'} por tramo.</div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(90px, 1fr))', gap: 8 }}>
                      {e.tramos.map((t, i) => (
                        <div key={i} style={{ border: '1px solid var(--line)', borderRadius: 'var(--r-md)', padding: '8px 10px', background: 'var(--surface-2)' }}>
                          ...
                          <input className="input" type="number" min="0" placeholder="0" value={t} onChange={ev => mutate(id, c => { c.tramos[i] = ev.target.value; })} style={{ fontVariantNumeric: 'tabular-nums', fontWeight: 700 }} />
                        </div>
                      ))}
                    </div>
                  </>
